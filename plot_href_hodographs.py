...
def download_file(date_str, run, fhr, prod_type):
    ...
    # Only print for 'mean' to reduce log spam
    if prod_type == 'mean':
...
def process_forecast_hour(date_obj, date_str, run, fhr):
    ...
    try:
        ...
        if pmmn_file:
            # Load Max UH (mxuphl) - 2-5km layer
            try:
                ds_pmmn = xr.open_dataset(pmmn_file, engine='cfgrib', 
                                          backend_kwargs={'filter_by_keys': {'shortName': 'mxuphl'}})
                if 'mxuphl' in ds_pmmn:
                    ds_uh_max = ds_pmmn['mxuphl']
                    print(f"       Found PMMN Max UH. Max: {ds_uh_max.values.max():.1f}")
            except: pass

            # Load Min UH (mnuphl) - 2-5km layer
            try:
                ds_pmmn_min = xr.open_dataset(pmmn_file, engine='cfgrib', 
                                              backend_kwargs={'filter_by_keys': {'shortName': 'mnuphl'}})
                if 'mnuphl' in ds_pmmn_min:
                    ds_uh_min = ds_pmmn_min['mnuphl']
                    print(f"       Found PMMN Min UH. Min: {ds_uh_min.values.min():.1f}")
            except: pass
        ...
        if ds_uh_max is not None:
            ...
            if np.nanmax(uh_vals) > 20: # Plot if > 20
        ...
        # 3. Plot MIN UH (PMMN) - Blue
        if ds_uh_min is not None:
            uh_min_vals = ds_uh_min.values
            if np.nanmin(uh_min_vals) < -20: # Plot if < -20
                ...
                # Min UH Colorbar (Right Bottom)
                ax_cbar_min = fig.add_axes([0.91, 0.15, 0.015, 0.35])
                plt.colorbar(min_plot, cax=ax_cbar_min, orientation='vertical', label='PMMN Min UH (<-20)')
