import cfgrib
import requests
import os
import sys
import datetime
import warnings

# Suppress warnings
warnings.filterwarnings("ignore")

def get_latest_run_time():
    now = datetime.datetime.utcnow()
    if now.hour >= 15:
        run = '12'
        date = now
    elif now.hour >= 3:
        run = '00'
        date = now
    else:
        run = '12'
        date = now - datetime.timedelta(days=1)
    return date.strftime('%Y%m%d'), run, date

def download_pmmn_file(date_str, run, fhr):
    # Note: Downloading PMMN file specifically
    base_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/href/prod/href.{date_str}/ensprod"
    filename = f"href.t{run}z.conus.pmmn.f{fhr:02d}.grib2"
    url = f"{base_url}/{filename}"
    
    print(f"\n[f{fhr:02d}] Downloading PMMN: {url}")
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'}
    
    try:
        with requests.get(url, stream=True, timeout=60, headers=headers) as r:
            if r.status_code == 404:
                print(f"File not found.")
                return None
            r.raise_for_status()
            with open(filename, 'wb') as f:
                for chunk in r.iter_content(chunk_size=8192):
                    f.write(chunk)
        return filename
    except Exception as e:
        print(f"Download failed: {e}")
        return None

def diagnose_grib_file(grib_file):
    print("\n" + "="*60)
    print(f"DIAGNOSTIC MODE: Scanning {grib_file}...")
    print("="*60)
    
    try:
        # Open all datasets in the file (ignoring filters)
        datasets = cfgrib.open_datasets(grib_file)
        
        for i, ds in enumerate(datasets):
            print(f"\n--- Dataset Part {i+1} ---")
            for var_name in ds.data_vars:
                da = ds[var_name]
                attrs = da.attrs
                
                short_name = attrs.get('GRIB_shortName', 'unknown')
                long_name = attrs.get('long_name', 'unknown')
                level_type = attrs.get('GRIB_typeOfLevel', 'unknown')
                
                # Extract level info
                level_desc = "N/A"
                if 'heightAboveGroundLayer' in da.coords:
                    level_desc = str(da['heightAboveGroundLayer'].values)
                elif 'isobaricInhPa' in da.coords:
                    level_desc = f"{da['isobaricInhPa'].values} mb"
                elif 'surface' in da.coords:
                    level_desc = "Surface"
                elif 'heightAboveGround' in da.coords:
                    level_desc = f"{da['heightAboveGround'].values} m"
                
                # Highlight Helicity
                is_helicity = 'helicity' in long_name.lower() or 'uphl' in short_name.lower()
                prefix = ">>> FOUND MATCH: " if is_helicity else "    "
                
                print(f"{prefix}Var: {var_name:<10} | Short: {short_name:<10} | Level: {level_type:<25} | Range: {level_desc}")

        print("\n" + "="*60)
        print("DIAGNOSTIC COMPLETE. Please paste the '>>> FOUND MATCH' lines here.")
        print("="*60)

    except Exception as e:
        print(f"Diagnostic failed: {e}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    date_str, run, date_obj = get_latest_run_time()
    run_dt = datetime.datetime.strptime(f"{date_str} {run}", "%Y%m%d %H")
    
    # Check f01 or f02 to find the variable
    filename = download_pmmn_file(date_str, run, 2)
    if filename:
        diagnose_grib_file(filename)
        if os.path.exists(filename):
            os.remove(filename)
